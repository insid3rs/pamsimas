...
class DocumentConverter:
    ...
    def convert(self, inputFile, inputFormat, outputFile, outputFormat):

        inputUrl = self._toFileUrl(inputFile)
        outputUrl = self._toFileUrl(outputFile)

        loadProperties = { "Hidden": True }
        if IMPORT_FILTER_MAP.has_key(inputFormat):
            loadProperties.update(IMPORT_FILTER_MAP[inputFormat])
        ...
        storeProperties = self._getStoreProperties(document, outputFormat)
    ...
    def _toFileUrl(self, path):
        cwd = uno.systemPathToFileUrl( os.getcwd() )
        fileUrl = uno.absolutize( cwd, uno.systemPathToFileUrl(path) )
        return fileUrl
...
if __name__ == "__main__":
    ...
    if len(argv) < 5:
        print "USAGE: python %s <input-file> <input-format> <output-file> <output-format>" % argv[0]
        exit(255)
    ...
    try:
        ...
        converter.convert(argv[1], argv[2], argv[3], argv[4])
